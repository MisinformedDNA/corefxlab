...
namespace System.Text
{
    ...
    public readonly struct UnicodeBlock : IEquatable<UnicodeBlock>
    {
        ...
        public UnicodeBlock(int firstCodePoint, int length)
        {
            ...
            uint firstCodePointUnsigned = (uint)firstCodePoint;
            firstCodePointUnsigned = (firstCodePointUnsigned >> 4) | (firstCodePointUnsigned << 28);
            if (firstCodePointUnsigned > 0x10FFF)
            {
                // TODO: Real exception message.
                throw new ArgumentOutOfRangeException(paramName: nameof(firstCodePoint));
            }

            // Check that length is in range [1, 65536] and is evenly divisible by 16.
            // Additionally, check that first code point + length doesn't go onto a new plane.
            uint lengthUnsigned = (uint)length;
            lengthUnsigned = (lengthUnsigned >> 4) | (lengthUnsigned << 28);
            if (!UnicodeHelpers.IsInRangeInclusive(lengthUnsigned, 1, 0x1000U - (firstCodePointUnsigned & 0xFFFU)))
            {
                // TODO: Real exception message.
                throw new ArgumentOutOfRangeException(paramName: nameof(length));
            }

            _value = (firstCodePointUnsigned << 13) | lengthUnsigned;
        }
